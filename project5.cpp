...
void ArrayBST<DT>::remove(DT &object)
{
	...
	while (cur != -1)
	{
		...
		else
		{
			...
			{
				if (last != -1 && dir == 1)
					_tree[last].setRight(_tree[cur].getright());
				if (last != -1 && dir == -1)
					_tree[last].setLeft(_tree[cur].getright());
				_tree[cur].setInfoNull();
				cur = last;
			}
			else if (L != -1 && R == -1)
			{
				if (last != -1 && dir == 1)
					_tree[last].setRight(_tree[cur].getleft());
				if (last != -1 && dir == -1)
					_tree[last].setLeft(_tree[cur].getleft());
				_freeLocations.push(cur);
				_tree[cur].setInfoNull();
				cur = last;
			}
			else
			{
				...
				if (cur == parent_p)
				{
					_tree[p].setLeft(_tree[cur].getleft());
					if (last != -1 && dir == 1)
						_tree[last].setRight(p);
					if (last != -1 && dir == -1)
						_tree[last].setLeft(p);
					_tree[cur].setInfoNull();
				}
				else
				{
					...
					_tree[p].setLeft(_tree[cur].getleft());
					_tree[p].setRight(_tree[cur].getright());
					if (last != -1 && dir == 1)
						_tree[last].setRight(p);
					if (last != -1 && dir == -1)
						_tree[last].setLeft(p);
